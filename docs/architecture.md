...
`digit-adapter` uses DIGIT's real `egov-workflow-v2` API (`RequestInfo`-wrapped process transitions) — verified against DIGIT's own API docs, not guessed — but routing a case requires a `BusinessService` to already exist on the target DIGIT instance (an operational setup step on DIGIT's side, out of this repo's scope). Real DIGIT is Kubernetes/Helm-first with no lightweight compose demo, so `digit-adapter` currently points at `digit-mock` (`services/digit-mock/`) — a small stand-in implementing the exact same two endpoints with an illustrative auto-progressing status, used for POC demos. Swapping `DIGIT_BASE_URL` for a real cluster later requires no code change on the adapter side.
